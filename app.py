...
# --- Destiny store (canon routes live in the private canon pack plugin) ---
from nougen_shards import destiny
...
def _nougenmsg_read(target: Optional[str] = None, limit: int = 10) -> dict:
    ...
    node_label = os.environ.get("NOUGEN_NODE_NAME") or machine.get_current_node()
    return {"complete": True, "node": node_label,
...
NODE_PLUGINS = _load_node_plugins(app, node_mcp, ctx={
    "offloaded": _offloaded,
    "ask_rhea_bounded": _ask_rhea_bounded,
    "tenant_vault_context": tenant_vault_context,
    "tenants": tenants,
})
