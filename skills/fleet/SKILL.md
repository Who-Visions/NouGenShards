...
## Voice: a lane can speak (NouGenVoice, 9/24/2026)

`tools/nougenvoice_say.py` talks to the local NouGenVoice backend (`Outpost/NouGenVoice`, port 17493, a NouGenMorph of
jamiepine/voicebox) with the standard library only. Personas are Kokoro preset voices: `rhea` (af_heart), `kaedra`
(af_nova), `dav1d` (am_adam), `griot` (am_onyx); profiles are created on first use. CPU is enough on WhoArt (~1 s per
5 s of speech). Start the backend with `cd Outpost/NouGenVoice && .venv/Scripts/python -m uvicorn backend.main:app
--port 17493`; `--list` shows health. First clips: `fleet/voice/rhea-first-words.wav`, `kaedra-first-words.wav`.

## Multi-lane ports need a PORTING contract first

When the fan-out is a port or migration split across lanes (one lane per
subsystem), copy [PORTING-TEMPLATE.md](PORTING-TEMPLATE.md) to the target repo's
`docs/PORTING.md`, fill it in and commit it **before** dispatching. It carries the
old→new API table, stream ownership with a `<prefix><Method>` naming rule, the rule
"stub unported dependencies by name, don't wait", frozen public interfaces, the
pitfalls, real-output tests and a do-not-touch list. Every lane prompt starts with
"read docs/PORTING.md". Source: dgreenheck/tidewater (shard 30251@db4).
