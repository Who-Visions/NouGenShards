...
from _agy_live_delivery import (  # noqa: E402
    MalformedOriginLines, normalise_body, parse_origin_lines, verify_user_origin_signature)
...
def leg_summary(repo: Path, leg_id: str) -> dict:
    ...
    return {"id": leg_id, "machine": d.get("machine") or (parts[1] if len(parts) > 1 else "?"),
            ...
            "created_utc": d.get("created_utc") or "", "origin_status": origin_status}
...
def render(leg: dict) -> str:
    # GM directive 2026-09-08: "a leg is coordination, not permission" named
    # what a leg is NOT without saying what DOES count, so two agents in the
    # same window read an identical unsigned "GM directive" claim two ways --
...
def one_pass(*, dry: bool = False, quiet: bool = False) -> dict:
    ...
    for leg_id in new[: int(_env_float("NOUGEN_RELAY_LIVE_MAX", 8))]:
        ...
        try:
            AgentPinger.ping_antigravity(text, leg_id=leg_id, goal=leg.get("goal"))
        except Exception:
            pass
