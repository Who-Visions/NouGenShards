...
function resolveConnectorLane(env, keyId) {
  const kid = String(keyId || "").toLowerCase();
  if (kid.startsWith("chatgpt") || kid.includes("chatgpt") || kid.startsWith("gpt-")) {
    return "chatgpt-app";
  }
  if (kid.startsWith("claude") || kid.includes("claude")) {
    return "claude-app";
  }
  if (kid.startsWith("gemini") || kid.includes("gemini") || kid.startsWith("agy")) {
    return "gemini-app";
  }
  return env?.CONNECTOR_LANE || "claude-app";
}
__name(resolveConnectorLane, "resolveConnectorLane");
function resolveShardLane(env, keyId) {
  const lane = resolveConnectorLane(env, keyId);
  if (lane === "chatgpt-app") return "chatgpt-client";
  if (lane === "claude-app") return "claude-client";
  if (lane === "gemini-app") return "gemini-client";
  return env?.SHARD_LANE || "claude-client";
}
__name(resolveShardLane, "resolveShardLane");
async function shardHeaders(env, keyId) {
  return {
    ...
    "x-nougen-lane": resolveShardLane(env, keyId)
  };
}
...
async function shardRpcHttp(env, method, params, id, keyId) {
  const res = await fetch(env.SHARD_GATEWAY_URL.replace(/\/$/, "") + "/mcp/", {
    method: "POST",
    headers: {
      ...await shardHeaders(env, keyId),
...
var HANDLERS = {
  ...
  async fleet_whoami(args, env, keyId) {
    const lane = resolveConnectorLane(env, keyId);
    const shardLane = resolveShardLane(env, keyId);
    const out = {
      key: keyId,
      lane,
    ...
    return text(
      `Authenticated as fleet key **${keyId}**, writing to the registry as lane **${lane}**.
...
` + truncate(body, "leg body truncated"),
  ...
  async relay_ack(args, env, keyId) {
    ...
    await ghWriteFile(
      ...
      `relay(${lane}): ack ${rec.goal || id}`,
      sha
    );
    return text(
      `\u2705 baton taken by ${lane}/${keyId}: ${rec.goal || id}`,
      { id, status: "acked" }
    );
  },
  async relay_create(args, env, keyId) {
    ...
    const id = `${stamp}__${lane}__${keyId}`;
    const record = {
      id,
      machine: lane,
      agent: keyId,
    ...
    const md = [
      `# \u{1F91D} Git Handoff \u2014 ${lane} / ${keyId}`,
      "",
      `**Goal**: ${args.goal}`,
    ...
    await ghWriteFile(
      ...
      `handoff(${lane}): ${args.goal}`
    );
    await ghWriteFile(
      env,
      `.handoffs/${id}.md`,
      md,
      `handoff(${lane}): ${args.goal} (body)`
    );
    return text(
      `\u2705 leg published: \`${id}\`
