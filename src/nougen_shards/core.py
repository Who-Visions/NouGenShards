...
def init_db(index: int = 1):
    ...
    try:
        ...
        # Temporal evidence v3 is additive: legacy timestamp remains the raw
        # record, while only trusted_event_timestamp may drive trusted history.
        for column, ddl in (
            ("temporal_meta", "TEXT"),
            ("original_timestamp", "TEXT"),
            ("raw_record_timestamp", "TEXT"),
            ("trusted_event_timestamp", "TEXT"),
            ("temporal_status", "TEXT DEFAULT 'UNVERIFIED'"),
            ("temporal_evidence_json", "TEXT"),
        ):
            try:
                cursor.execute(f"ALTER TABLE shards ADD COLUMN {column} {ddl};")
            except sqlite3.OperationalError:
                pass
...
def _get_dedup_connection():
    ...
    """
    conn = sqlite3.connect(str(get_dedup_path()), timeout=60.0)
    conn.execute("PRAGMA journal_mode=WAL;")
    conn.execute("PRAGMA busy_timeout=60000;")
    conn.execute("PRAGMA synchronous=NORMAL;")
    conn.execute("""
...
def normalize_temporal_meta(temporal_meta=None, original_timestamp=None, event_time_original=None,
                            source_created_at=None, source_modified_at=None, captured_at=None,
                            ai_first_touched_at=None, ai_last_touched_at=None, migrated_at=None,
                            amended_at=None, now_iso=None) -> dict:
    """Normalizes and canonicalizes Schema v3 temporal provenance vector metadata."""
    if isinstance(temporal_meta, dict):
        d = dict(temporal_meta)
    elif isinstance(temporal_meta, str) and temporal_meta.strip():
        try:
            d = json.loads(temporal_meta)
        except Exception:
            d = {"raw_meta": temporal_meta}
    else:
        d = {}

    now = now_iso or datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    if original_timestamp and "original_timestamp" not in d:
        d["original_timestamp"] = original_timestamp
    if event_time_original and "event_time_original" not in d:
        d["event_time_original"] = event_time_original
    if source_created_at and "source_created_at" not in d:
        d["source_created_at"] = source_created_at
    if source_modified_at and "source_modified_at" not in d:
        d["source_modified_at"] = source_modified_at
    if captured_at and "captured_at" not in d:
        d["captured_at"] = captured_at
    else:
        d.setdefault("captured_at", now)
    if ai_first_touched_at:
        d["ai_first_touched_at"] = ai_first_touched_at
    if ai_last_touched_at:
        d["ai_last_touched_at"] = ai_last_touched_at
    if migrated_at:
        d["migrated_at"] = migrated_at
    if amended_at:
        d["amended_at"] = amended_at
...
def capture(event_type: str, title: str, content: str,
    ...
    publishers. Provenance is retained in `temporal_meta`, while `utility` seeds
    ...
    try:
        ...
        if source_uri is not None:
            t_dict["source_uri"] = redact_content(str(source_uri))
        t_dict.setdefault("raw_record_timestamp", original_timestamp or timestamp)
        from .temporal_evidence import assess_temporal_evidence  # local: no capture cycle
        temporal_assessment = assess_temporal_evidence(
            title=title, content=content, metadata=t_dict)
        t_dict.update(temporal_assessment.as_dict())
        temporal_meta_str = json.dumps(t_dict)
        temporal_evidence_str = json.dumps(temporal_assessment.as_dict())
        ...
        for attempt in range(5):
            ...
            try:
                cursor = conn.execute("""
                    INSERT INTO shards (timestamp, event_type, title, content, tags, file_hash, embedding, domain_key, density_score, sensitivity, enc, temporal_meta, original_timestamp, raw_record_timestamp, trusted_event_timestamp, temporal_status, temporal_evidence_json, utility_score)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (timestamp, event_type, title, stored_content, tags_str, fhash, emb_blob, domain_key, density_score, sensitivity, enc_flag, temporal_meta_str, original_timestamp, t_dict.get("raw_record_timestamp"), temporal_assessment.trusted_event_timestamp, temporal_assessment.status.value, temporal_evidence_str, utility_score))
                conn.commit()
...
_VECTOR_CACHE: dict = {}
_VECTOR_CACHE_LOCK = None  # created lazily; one lock guards load/refresh


def _vector_cache_lock():
    global _VECTOR_CACHE_LOCK  # pylint: disable=global-statement
...
def _vector_cache_entry(i: int, conn) -> Optional[dict]:
    ...
    with _vector_cache_lock():
        ...
        if entry is not None:
            fresh = {
                "sig": sig, "dim": dim, "n_embedded": n_embedded, "max_id": max_id,
