...
The Keymaker (`keymaker.py`) protects *secrets* â€” API keys, service accounts â€” with
...
path â€” if the recovery file cannot be written, key generation fails closed.
...
def _candidate_key_paths() -> list:
    ...
    A deployment can legitimately have more than one "vault" directory â€” an
...
def _write_recovery_key(raw: bytes) -> str:
    ...
    body = (
        "NouGen private vault â€” RECOVERY KEY\n"
        "===================================\n\n"
