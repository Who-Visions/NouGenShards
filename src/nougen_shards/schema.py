...
# v3 -- machine provenance: stamps which fleet node (hostname) originated each
# shard. Existing rows default to '' (empty); the backfill script stamps them
# with the node's hostname after the migration runs. New shards written by
# capture() are stamped at insert time automatically.
_SHARD_COLUMNS_V3 = [
    ("machine", "TEXT DEFAULT ''"),
]

_SHARD_INDEXES_V3 = [
    ("idx_shards_machine", "CREATE INDEX IF NOT EXISTS idx_shards_machine ON shards (machine)"),
]
...
MIGRATIONS: List[Migration] = [
    ...
    Migration(3, "machine-provenance-column", _plan_v3, _apply_v3),
]
