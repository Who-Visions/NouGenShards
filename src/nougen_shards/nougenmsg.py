...
class NouGenMsgBus:
    ...
    #: Anchors deliberately omitted -- this is used with `fullmatch` only.
    #: Leaving `^...$` in place invites a future edit back to `.match()`, which
    #: is exactly the trailing-newline hole that was closed here.
    ...
    def emit_node(cls, node: str, target: str, text: str) -> Dict[str, Any]:
        ...
        for label, value in (("node", node), ("target", target)):
            ...
            # trailing newline, so `.match()` accepted a node name with a
            # trailing newline as valid. Closed on blade and phoebus on
            # 2026-09-04; the whoart copy was still on `.match()`.
