...
def test_privacy_mode_drops_cloud_and_vertex_at_load(tmp_path, monkeypatch):
    ...
    # Fresh installs have no implicit local peer routes; provide one explicit
    # tenant-local route so this test isolates privacy filtering behavior.
    ...
    assert f.routes == [fleet.LOCAL_ROUTES[0]]
    assert all(fleet.is_private_url(r["url"]) for r in f.routes)
