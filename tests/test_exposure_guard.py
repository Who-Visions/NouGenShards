...
class TestStartupLogging:
    ...
    def test_decision_is_immutable(self):
        ...
        # Specifically FrozenInstanceError -- a bare `Exception` (or even
        # AttributeError, which this subclasses) would also be satisfied by a
        # typo'd attribute name and prove nothing about frozenness.
        with pytest.raises(dataclasses.FrozenInstanceError):
            decision.exposed = False  # frozen dataclass
        with pytest.raises(dataclasses.FrozenInstanceError):
            del decision.exposed

        # The verdict genuinely survived the attempts.
        assert decision.exposed is True
