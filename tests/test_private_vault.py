...
These tests pin the properties that make that safe:
  ...
  4. normal shards are untouched — no re-encryption, no behaviour change;
...
def test_round_trip_preserves_unicode():
    secret = "Placeholder Bank ending 0000 — balance $1.23 · café"
...
def test_private_body_absent_from_fts_index(vault):
    ...
    assert indexed, "the FTS trigger should still fire — only the body is opaque"
...
def test_existing_key_is_found_before_a_new_one_is_minted(tmp_path, monkeypatch):
    ...
    other configured locations before generating — otherwise a second key gets
