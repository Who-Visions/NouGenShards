...
class TestCLI(unittest.TestCase):
    ...
    @patch('nougen_shards.cli.keymaker.init_vault')
    @patch('nougen_shards.cli.shards.init_db')
    def test_cmd_init_builds_the_full_grid(self, mock_init, _vault):
        """init provisions all 9 shard DBs, not just the first.

        Two cmd_init definitions coexisted in this module after a concurrent
        merge; Python bound the later one, so the earlier implementation and
        the tests written against it were both dead. These assert the SURVIVING
        contract instead of any one banner string.
        """
        args = argparse.Namespace(json=False, defaults=True, no_onboarding=True)
        with patch('sys.stdout', new=io.StringIO()) as fake_out:
            cli.cmd_init(args)
        self.assertEqual(mock_init.call_count, cli.shards.MAX_DB_COUNT)
        self.assertIn("substrate initialized", fake_out.getvalue().lower())

    @patch('nougen_shards.cli.keymaker.init_vault')
    @patch('nougen_shards.cli.shards.init_db')
    def test_cmd_init_json_is_machine_readable(self, _init, _vault):
        """--json must emit parseable JSON, not decorated text."""
        args = argparse.Namespace(json=True, defaults=True, no_onboarding=True)
        with patch('sys.stdout', new=io.StringIO()) as fake_out:
            cli.cmd_init(args)
    ...
    @patch('nougen_shards.cli.keymaker.init_vault')
    @patch('nougen_shards.cli.shards.init_db')
    def test_cmd_init_no_onboarding_skips_discovery(self, mock_init, _vault):
        ...
        with patch('sys.stdout', new=io.StringIO()) as fake_out:
            cli.cmd_init(args)
        # The human path prints the grid line; "onboarding skipped" lives in
        # the JSON message, so assert the behaviour, not the other view's text.
        self.assertIn("substrate initialized", fake_out.getvalue().lower())
        self.assertEqual(mock_init.call_count, cli.shards.MAX_DB_COUNT)
